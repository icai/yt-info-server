...
http.createServer(async function (request, response) {
  ...
  if (!ytdl.validateURL(vidId)) { // URL is not valid
    ...
    response.writeHead(404, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': allowDomain });
    if (vidId == "") {
    ...
    ytdl.getInfo(vidId).then((value) => { // Promise fulfilled
      ...
      response.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': allowDomain });
